...
def observe():
    ...
    revision=int(hashlib.sha256(json.dumps([(p['port'],p['available'],p['enabled']) for p in ports]).encode()).hexdigest()[:12],16)
    return {'revision':revision,'ports':ports,'capabilities':{'admin_state':True,'speed_configuration':False,
            'link_is_forwarding':False},'saved':json.loads(STATE.read_text()) if STATE.exists() else {'ports':{}}}


def apply(request):
    if (not isinstance(request,dict) or set(request)!={'revision','port','enabled'} or
            type(request['revision']) is not int or type(request['port']) is not int or
            not 1<=request['port']<=24 or type(request['enabled']) is not bool):
    ...
    save(saved)
    call({'op':'port.set','port':port['bcm_port'],'enable':request['enabled']})
    after=observe()
    if after['ports'][request['port']-1]['enabled'] != request['enabled']:
        raise RuntimeError('administrative state readback did not match; operation pending')
    saved['ports'][str(request['port'])]=request['enabled']
    saved.pop('pending')
    save(saved)
    after['saved']=saved
...
def main():
    with open('/run/ffn-faceplate.lock','w') as lock:
        ...
        if action=='status': result=observe()
            ...
            if not port['available']: raise ValueError('port unavailable')
            state['ports'][str(pending['port'])]=port['enabled']
