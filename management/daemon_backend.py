...
FIELDS={'phy':{'revision','phy','speed'},'bcm':{'revision','operation','acknowledge_link_outage'},'network':{'revision','ports','routes','vrfs','rules'},
        ...
        'faceplate':{'revision','port','enabled','speed','restart_autoneg'},'thermal':{'revision','operation'}}
    ...
    if action in ('apply','validate'):
        ...
        if resource=='thermal':
            ...
            if resource=='faceplate':
                if (not {'port','revision'}<=set(payload) or not {'enabled','speed','restart_autoneg'}&set(payload) or type(payload['port']) is not int or
