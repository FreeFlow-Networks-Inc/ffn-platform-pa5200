/* Initialize front-panel SerDes with every data port administratively down.
 ...
 */
{
    ...
    for (fp_i = 0; fp_i < 24; fp_i++) {
        fp_rv = bcm_port_enable_set(0, fp_ports[fp_i], 0);
        printf("FFN_FRONT disable port=%d rv=%d\n", fp_ports[fp_i], fp_rv);
        ...
        if (fp_rv != 0) { fp_errors++; }
        fp_rv = bcm_port_enable_set(0, fp_ports[fp_i], 0);
        printf("FFN_FRONT disabled port=%d rv=%d\n", fp_ports[fp_i], fp_rv);
        if (fp_rv != 0) { fp_errors++; }
    }
