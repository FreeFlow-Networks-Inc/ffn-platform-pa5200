...
PORTS = {
    ...
    25: ("XLGE13", 0, 25, "TM",  "RAW",      DP,      "second DP link. MEASURED down with signal_detect=0 and lock=0 on ALL FOUR LANES, while our own SerDes is healthy (UC_ATV=1, PLL_LOCK=1*, ucode D10F_13, VCO 10.25GHz). So this end is trained and listening and the far end is not transmitting -- nothing is wrong on the switch side. That it is a DP link at all is inferred, not measured: only port 24 was confirmed against the DP"),
    26: ("XLGE10", 0, 26, "TM",  "RAW",      DP,      "third DP link. MEASURED down, identical to 25: signal_detect=0 and lock=0 on all four lanes with a healthy local SerDes"),
